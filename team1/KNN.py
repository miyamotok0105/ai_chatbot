...
print "Training the random KNN..."

# Initialize a Random KNN classifier with 100 trees
...
KNN = KNN.fit( train_data_features, train["label"] )

print('accuracy on the training set: %f' %KNN.score( train_data_features, train["label"] ))
...
# Use the random KNN to make sentiment label predictions
result = KNN.predict(test_data_features)
...
for i in xrange(0,num_test_data):
    if result[i] == test['label'][i]:
        ...
        badPrediction += 1
        print('sample = %s pred = %s, correct = %s confidence = %f' % (test['text'][i], result[i] , test['label'][i], np.amax(KNN.predict_proba(train_data_features[i].reshape(1,-1)))))
