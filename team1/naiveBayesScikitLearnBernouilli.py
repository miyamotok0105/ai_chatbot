...
log = logging.getLogger('randomNB')
...
# Use the random NB to make sentiment label predictions
...
for i in xrange(0,num_test_data):
    if result[i] == test['label'][i]:
        ...
        badPrediction += 1
        print('sample = %s pred = %s, correct = %s confidence = %f' % (test['text'][i], result[i] , test['label'][i], np.amax(NB.predict_proba(train_data_features[i].reshape(1,-1)))))
